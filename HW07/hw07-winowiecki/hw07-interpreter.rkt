...
(define (value-of-program prog env)
  (cases program prog
    (a-program
     ...
     (andmap (lambda (ex) (value-of ex (foldl value-of env fun-defs)))
...
(define (value-of-expr ex env)
  ...
  (cases expr ex
    ...
    ;New expressions
    (fun-expr (vars body) (proc-val (procedure vars body env)))
    (fun-call-expr (expr rest-of-expr)
                   (letrec
                       ([fun (proc-val->proc (value-of expr env))]
                        [expr-vals (map (lambda (x) (value-of x env)) rest-of-expr)]
                        [names (proc->vars fun)]
                        [body (proc->body fun)]
                        [penv (proc->env fun)]
                        [new-env
                         (begin
                           (or (equal? (length names) (length expr-vals))
                               (raise (~a "arity mismatch. expected " (length names) " arguments, received " (length expr-vals))))
                           (foldl (lambda (sym val env) (extend-env-wrapper sym val env FINAL))
                                  penv
                                  names
                                  expr-vals))])
...
(define (value-of-var v-ex env)
  ...
  (cases var-expr v-ex
    ...
    ;New var-expression
    (fun-def (name params body)
             (let
                 ([rec-env (extend-env-rec name params body env)])
               (extend-env-wrapper name (proc-val (procedure params body rec-env)) env FINAL)))
